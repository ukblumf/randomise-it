...
class User(UserMixin, db.Model):
    ...
    share_ban = db.Column(db.Boolean, default=False)
    share_ban_until = db.Column(db.DateTime())
    share_ban_description = db.Column(db.String(512))
    login_ban = db.Column(db.Boolean, default=False)
    login_ban_until = db.Column(db.DateTime())
    login_ban_description = db.Column(db.String(512))
    ...
    def __init__(self, **kwargs):
        ...
        if self.role is None:
            if self.email == current_app.config['RANDOMIST_ADMIN']:
